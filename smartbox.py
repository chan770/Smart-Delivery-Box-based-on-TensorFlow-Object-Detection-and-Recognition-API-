...
to_whatsapp_number='whatsapp:+xxxxxxxxxxxxxx'
msg= 'Element found at  ' + datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")

clientsms = Client(account_sid,auth_token)
clientwhats = Client(account_sid,auth_token)
# EMAIL SETUP
mail = {
    'subject': 'SMART POSTBOX!',  # Anything you want.
    'content_text': msg,  
    'attachments': ['/home/pi/tensorflow1/models/research/object_detection/box_pic/element.jpg'],  
}
...
PATH_TO_LABELS = os.path.join(CWD_PATH,'data','smartbox.pbtxt')
...
TL_inside = (int(IM_WIDTH*0.1),int(IM_HEIGHT*0.1))
BR_inside = (int(IM_WIDTH*0.9),int(IM_HEIGHT-5))
...
def info(gotallx):
        print (gotallx)
        print('take a pic')
        camera.capture('/home/pi/tensorflow1/models/research/object_detection/box_pic/element.jpg')
        print ('sending mail')
        server.send_mail(tosendemail, mail)
        print ('sending sms')
        clientsms.messages.create(body = msg,from_=twilio_number,to=my_number)
        print ('sending whatsapp msg')
        clientwhats.messages.create(body=msg,from_=from_whatsapp_number,to=to_whatsapp_number)
        os.system('rm test.txt')
        file=open("test.txt","w")
        file.write(str(gotallx))
        file.close()
        time.sleep(2)
        exit
...
def pet_detector(frame):
    ...
    cv2.rectangle(frame,TL_inside,BR_inside,(20,20,255),1)
    #cv2.putText(frame,"Inside box",(TL_inside[0]+10,TL_inside[1]-10),font,1,(20,255,255),1,cv2.LINE_AA)
    ...
    for i in range (1,6):
        print ('i value in for : '+str(i))
        if ((int(classes[0][0]) == i)):
    ...
    # Draw counter info
        #cv2.putText(frame,'Detection counter: ' + str(max(a[i],outside_counter)),(10,100),font,0.5,(255,255,0),1,cv2.LINE_AA)
    return frame
...
if camera_type == 'picamera':
    ...
    #frame=frame1.array
        ...
        # Draw FPS
        #cv2.putText(frame,"FPS: {0:.2f}".format(frame_rate_calc),(30,50),font,1,(20,255,255),1,cv2.LINE_AA)
